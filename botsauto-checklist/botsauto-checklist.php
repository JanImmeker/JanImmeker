...
class BOTSAUTO_Checklist {
    ...
    public function meta_box_lines( $post ) {
        ...
        echo '<style>#botsauto-editor p{display:flex;align-items:center;gap:6px;margin:4px 0;flex-wrap:wrap;}#botsauto-editor label{flex:1;display:flex;align-items:center;min-width:0;color:' . esc_attr($s['primary']) . ';}#botsauto-editor label span{display:inline-block;width:140px;}#botsauto-editor input, #botsauto-editor textarea{flex:1;width:' . esc_attr($adv['field']['width']) . ';max-width:none;border-radius:' . esc_attr($adv['field']['border-radius']) . ';border-style:' . esc_attr($adv['field']['border-style']) . ';border-width:' . esc_attr($adv['field']['border-width']) . ';border-color:' . esc_attr($adv['field']['border-color']) . ';background:' . esc_attr($adv['field']['background-color']) . ';color:' . esc_attr($adv['field']['text-color']) . ';box-sizing:border-box;}#botsauto-editor .question-field{max-width:calc(100% - 220px);}#botsauto-editor .question-line{margin-left:2em;}#botsauto-editor .info-line{margin-left:2em;margin-bottom:0.5em;display:block;width:100%;}#botsauto-editor .item-line{margin-left:4em;}#botsauto-editor{background:' . esc_attr($s['background']) . ';color:' . esc_attr($s['text']) . ';font-family:' . esc_attr($s['font']) . ';}#botsauto-editor input[type=checkbox]{accent-color:' . esc_attr($s['primary']) . '!important;display:inline-block!important;width:auto!important;height:auto!important;appearance:auto!important;}#botsauto-editor .button{background:' . esc_attr($s['primary']) . ';border-color:' . esc_attr($s['primary']) . ';color:#fff;}#botsauto-editor .botsauto-phase>details>summary{font-weight:bold;cursor:pointer;margin:0;color:' . esc_attr($s['primary']) . ';list-style:none;position:relative;padding-left:1.2em;}#botsauto-editor .botsauto-phase>details>summary::before{content:"▶";position:absolute;left:0;}#botsauto-editor .botsauto-phase>details[open]>summary::before{content:"▼";}</style>';
    }
...
CHECKLIST;
    ...
    public function render_form( $atts ) {
        ...
        foreach ( $items_to_use as $hash => $data ) {
            ...
            echo '<li>';
            if ( $data['question'] ) {
                echo '<div class="botsauto-question-row"><span class="botsauto-question-label">'.esc_html( $data['question'] ).'</span>';
                if ( !empty($data['info']['text']) || !empty($data['info']['url']) ) {
                    echo ' <details class="botsauto-info"><summary class="botsauto-info-btn">'.esc_html__('Meer info','botsauto-checklist').'</summary></details>';
                }
                echo '</div>';
                if ( !empty($data['info']['text']) || !empty($data['info']['url']) ) {
                    echo '<div class="botsauto-info-content">';
                    if ( $data['info']['text'] ) {
                        echo '<p>'.esc_html( $data['info']['text'] ).'</p>';
                    }
                    if ( $data['info']['url'] ) {
                        echo '<p><a href="'.esc_url( $data['info']['url'] ).'" target="_blank">'.esc_html( $data['info']['url'] ).'</a></p>';
                    }
                    echo '</div>';
                }
            }
            ...
            echo '<div class="botsauto-answer-row"><input type="checkbox" id="'.$cid.'" class="botsauto-checkbox" name="answers['.$hash.']" '.$checked.'> <label for="'.$cid.'">'.esc_html( $data['item'] ).'</label> <button type="button" class="'.$cls.'"><i class="fa '.$icon.'"></i></button><div class="botsauto-note" style="display:none"><textarea class="botsauto-rich" name="notes['.$hash.']">'.$note.'</textarea></div></div>';
            echo '</li>';
        }
        if ( $open_ul ) {
            echo '</ul></details>';
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  document.querySelectorAll('#{$wrapper} details.botsauto-info').forEach(function(d){
    var content=d.parentElement.nextElementSibling;
    if(content && content.classList.contains('botsauto-info-content')){
      content.style.display=d.hasAttribute('open')?'block':'none';
      d.addEventListener('toggle',function(){content.style.display=this.open?'block':'none';});
    }
  });
  ...
  form.addEventListener('submit',function(e){
    ...
    private function generate_pdf( $title, $name, $answers, $snapshot, $notes = array(), $image = '' ) {
        ...
        foreach ( $snapshot as $hash => $item ) {
            ...
            if ( $item['question'] ) {
                ...
                if ( !empty( $item['info']['text'] ) ) {
                    $pdf->SetFont( $pdf_font, '', 9 );
                    $pdf->MultiCell(0, 6, $this->pdf_string( strip_tags( $item['info']['text'] ) ), 0, 'L');
                }
                if ( !empty( $item['info']['url'] ) ) {
                    $pdf->SetFont( $pdf_font, '', 9 );
                    $pdf->MultiCell(0, 6, $this->pdf_string( $item['info']['url'] ), 0, 'L');
                }
            }
    ...
    private function build_css( $style, $adv, $selector, $custom = '' ) {
        ...
        $css .= "$selector .botsauto-info-content{display:none;background:{$adv['info_popup']['background-color']};color:{$adv['info_popup']['text-color']};padding:{$adv['info_popup']['padding']};border-radius:{$adv['info_popup']['border-radius']};margin:0 0 .25em;}";
        $css .= "$selector details.botsauto-info>summary{display:inline-block;cursor:pointer;margin-left:.5em;white-space:nowrap;}";
        $css .= "$selector details.botsauto-info>summary::-webkit-details-marker{display:none;}";
        $css .= "$selector details.botsauto-info>summary::marker{content:'';font-size:0;}";
    ...
    public function output_frontend_style() {
        ...
        echo '<style class="botsauto-style">'
            ...
            . '.botsauto-answer-row{display:flex;align-items:center;gap:.5em;}'
